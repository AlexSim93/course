import { MOVIES_ARE_LOADING, MOVIES_HAVE_ERROR, MOVIES_FETCH_DATA_SUCCESS, RESET_MOVIES, REMOVE_FROM_MOVIES } from '../actions/index';
...
const entities = (state = initialState, action: any) => {
    switch (action.type) {
        ...
        case REMOVE_FROM_MOVIES:
          return {fetching: false, error: false, movies: state.movies.filter((movie: any) => movie.id !== action.payload)};
        default:
          return state;
    }
}; 
